...
<body>
  <div class="container">
    ...
    <section class="smote p-5">
      <h3 class="mt-5 mb-5 text-center">Synthetic Minority Oversampling Technique (SMOTE)</h3>
    ...
    <section class="evaluasi svm p-5">
      <?php
      ...
      $br = array();           //nyimpen data bangkrut
      $tb = array();           //nyimpen data tidak bangkrut
      $abc = array();
      $no = 0;
      $u = 0;
      foreach ($db as $a) {
      ...
      $uji_br = array();
      $uji_tb = array();
      for ($i = count($br) - 1; $i >= count($br) - 5; $i--) {
        array_push($uji_br, $br[$i]);
        array_push($uji_tb, $tb[$i]);
      }
      for ($i = 0; $i < 5; $i++) {
        array_pop($br);
        array_pop($tb);
      ...
      $uji1 = array();          // buat nyimpen data uji
      // echo '<pre>';
      // print_r($datacek);
      // echo '</pre>';
      // die();
      foreach ($datacek as $u) {
        array_pop($u);
        array_push($uji1, $u);
      }
      ...
      $svm = new Svm();
      $svm->train($ab, $l);
      $prediksi = $svm->predict($uji1);

      ?>

      <h3>Prediksi Dengan SVM</h3>
      <h6>Data Testing</h6>
      <div class="table-responsive">
